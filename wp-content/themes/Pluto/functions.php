...
// PURITAN & CO SPECIFIC FUNCTIONS HERE 
...
function my_mce_before_init( $settings ) {
    $style_formats = array(
        ...
        array(
        	'title' => 'menu item desc',
    		'inline' => 'span',
    		'classes' => 'menu-item-desc'
        ),
    	array(
    		'title' => 'menu feature section',
    		'block' => 'div',
    		'classes' => 'menu-feature-section',
			'wrapper' => true
    	)
    );
...
// get single homepage gallery image
function function_get_page_bk() {
	$args = array( 
	...
	if ( $attachments ) {
		...
		//	echo wp_get_attachment_image( $attachment->guid, 'full' );
		//}
		?>
        <style>
            body {
                background: url(<?php echo $attachments[0]->guid; ?>);
                filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src='<?php echo $attachments[0]->guid ?>', sizingMethod='scale');
                -ms-filter: "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='<?php echo $attachments[0]->guid ?>',sizingMethod='scale')";
...
add_action('get_page_bk','function_get_page_bk');
